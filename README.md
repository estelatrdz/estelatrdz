# Hola, soy Estela Trujillo 👋

## Sobre mí

🎯 Business Analyst con más de 20 años de experiencia en operaciones retail en CDMX.
📊 Especializada en análisis de procesos, requerimientos funcionales y toma de decisiones basada en datos.
🚀 Combino experiencia operativa real con herramientas de análisis para traducir datos en decisiones concretas de negocio.
🔍 En este perfil encontrarás proyectos de Power BI, SQL y análisis de datos con enfoque en contexto de negocio real.

## 🛠️ Herramientas y tecnologías

- **Análisis de negocios**: Business Analysis · BPMN · Gap Analysis · User Stories · Requerimientos funcionales
- **Datos y visualización**: SQL · Power BI · Python · DAX
- **Gestión de proyectos**: Jira · Confluence · Scrum · Kanban
- **Modelado de procesos**: Bizagi · Visio

## 📂 Proyectos destacados

### 🔹 [Dashboard Caso Dos Pinos — KPIs de ventas y distribución](https://github.com/estelatrdz/PBI_Projects/tree/main/KPI's_DosPinos)
Dashboard interactivo en Power BI desarrollado en el evento "Inmersión en Análisis de Datos" de Grow Up Data Analytics. Transforma datos complejos en herramientas de decisión con KPIs de ventas, distribución e insights accionables.

### 🔹 [Dashboard Turismo Costa Rica — Análisis de KPIs](https://github.com/estelatrdz/PBI_Projects/tree/main/Tourism_KPI's)
Análisis de comportamiento de visitantes, tendencias por región y estacionalidad del sector turístico de Costa Rica. Dashboard con KPIs e insights para apoyo a decisiones estratégicas.

### 🔹 [Reto Grow Up Data Analytics — Dashboard de KPIs](https://github.com/estelatrdz/PBI_Projects/blob/main/Reto%20Grow%20UP.pbix)
Tablero interactivo para monitoreo de KPIs críticos de negocio, diseñado para facilitar la toma de decisiones estratégicas en tiempo real.

### 🔹 [Reto Fútbol 2026 — Copa Mundial de Fútbol]((https://github.com/estelatrdz/PBI_Projects/tree/main/Reto_Copa_Mundial )

Dashboard interactivo en Power BI con 3 pantallas sobre la Copa Mundial de Fútbol 2026. Modelo de datos en esquema estrella (6 tablas), con insights validados directamente contra los datasets fuente. Desarrollado como Reto de 14 días de Grow Up Data Analytics.

## 📫 Contacto

- 📧 estela.trujillo.rdz@gmail.com
- 🔗 [LinkedIn: estela-trujillordz](https://www.linkedin.com/in/estela-trujillordz)
